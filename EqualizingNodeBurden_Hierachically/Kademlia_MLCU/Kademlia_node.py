...
class Kademlia_node():
    ...
    def __init__(self, ip, port, layer):
        self.ip = ip
        self.port = port
    ...
    def lookup_value(self, key):
        ...
        # Consider Hierarchically
        node_list = [n for n in node_list if n.layer <= self.layer]
        for n in node_list:
            if n.layer > self.layer: raise Exception
        node_list = node_list[:self.beta]
    ...
    def publish_to_LowFreqStorage(self, key, value):
        ...
        for n in node_list:
            if n.layer > self.layer: raise Exception
        node_list = node_list[:self.beta]
        # if len(node_list) != self.beta:
        #     print(len(node_list), self.layer)
        #     raise Exception

...
class access_pattern:
    ...
    def Access(cls, nodes):
        ...
        elif cls.pattern == "DATA_CLASS":
            return cls.DataClassAccessPattern(nodes)
        else:
    ...
    def DataClassAccessPattern(cls, nodes):
        total_access = 0
        remote_access = 0
        for n in nodes:
            for i in range(cls.num):
                total_access += 1
                data_class = random.choices(range(len(n.accessProbabilityList)), weights=n.accessProbabilityList, k=1)[0]
                try:
                    idx = random.choice(objectchain.data_class[data_class])
